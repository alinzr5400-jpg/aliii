...
export const BatchDeployNfts = {
    ...
    fromSlice(s: c.Slice): BatchDeployNfts {
        ...
        return {
            $: 'BatchDeployNfts',
            queryId: s.loadUintBig(64),
            deployList: c.Dictionary.load<uint64, BatchDeployDictItem>(c.Dictionary.Keys.BigUint(64), createDictionaryValue<BatchDeployDictItem>(BatchDeployDictItem.fromSlice, BatchDeployDictItem.store), s),
        }
    },
    store(self: BatchDeployNfts, b: c.Builder): void {
        b.storeUint(0x00000002, 32);
        b.storeUint(self.queryId, 64);
        b.storeDict<uint64, BatchDeployDictItem>(self.deployList, c.Dictionary.Keys.BigUint(64), createDictionaryValue<BatchDeployDictItem>(BatchDeployDictItem.fromSlice, BatchDeployDictItem.store));
    },
...
/**
 > struct (0x10000001) EnableReveal {
 >     queryId: uint64
 > }
 */
export interface EnableReveal {
    readonly $: 'EnableReveal'
    queryId: uint64
}
...
export const EnableReveal = {
    ...
    fromSlice(s: c.Slice): EnableReveal {
    ...
    store(self: EnableReveal, b: c.Builder): void {
        b.storeUint(0x10000001, 32);
        b.storeUint(self.queryId, 64);
    },
    toCell(self: EnableReveal): c.Cell {
        return makeCellFrom<EnableReveal>(self, EnableReveal.store);
    }
}

/**
 > struct (0x10000002) DisableReveal {
 >     queryId: uint64
 > }
 */
export interface DisableReveal {
    readonly $: 'DisableReveal'
    queryId: uint64
}

export const DisableReveal = {
    PREFIX: 0x10000002,
    ...
    fromSlice(s: c.Slice): DisableReveal {
    ...
    store(self: DisableReveal, b: c.Builder): void {
        b.storeUint(0x10000002, 32);
        b.storeUint(self.queryId, 64);
    },
    toCell(self: DisableReveal): c.Cell {
        return makeCellFrom<DisableReveal>(self, DisableReveal.store);
    }
}
...
export const OffchainMetadataReply = {
    ...
    toCell(self: OffchainMetadataReply): c.Cell {
        return makeCellFrom<OffchainMetadataReply>(self, OffchainMetadataReply.store);
    }
}
...
export class AlamdarCollection implements c.Contract {
    ...
    static fromStorage(emptyStorage: {
        ...
        const initialState = {
            ...
            data: NftCollectionStorage.toCell(NftCollectionStorage.create(emptyStorage)),
        };
    ...
    static createCellOfEnableReveal(body: {
        ...
        return EnableReveal.toCell(EnableReveal.create(body));
    }

    static createCellOfDisableReveal(body: {
        queryId: uint64
    }) {
        return DisableReveal.toCell(DisableReveal.create(body));
    }
    ...
    async sendRequestRoyaltyParams(provider: ContractProvider, via: Sender, msgValue: coins, body: {
        queryId: uint64
    ...
    async sendDeployNft(provider: ContractProvider, via: Sender, msgValue: coins, body: {
        queryId: uint64
        itemIndex: uint64
        attachTonAmount: coins
        initParams: CellRef<NftItemInitAtDeployment>
    ...
    async sendBatchDeployNfts(provider: ContractProvider, via: Sender, msgValue: coins, body: {
        queryId: uint64
        deployList: c.Dictionary<uint64, BatchDeployDictItem>
    }, extraOptions?: ExtraSendOptions) {
        return provider.internal(via, {
    ...
    async sendChangeCollectionAdmin(provider: ContractProvider, via: Sender, msgValue: coins, body: {
        queryId: uint64
        newAdminAddress: c.Address
    }, extraOptions?: ExtraSendOptions) {
        return provider.internal(via, {
    ...
    async sendEnableReveal(provider: ContractProvider, via: Sender, msgValue: coins, body: {
        queryId: uint64
    }, extraOptions?: ExtraSendOptions) {
        return provider.internal(via, {
            value: msgValue,
            body: EnableReveal.toCell(EnableReveal.create(body)),
            ...extraOptions
        });
    }

    async sendDisableReveal(provider: ContractProvider, via: Sender, msgValue: coins, body: {
        queryId: uint64
    }, extraOptions?: ExtraSendOptions) {
        return provider.internal(via, {
            value: msgValue,
            body: DisableReveal.toCell(DisableReveal.create(body)),
            ...extraOptions
        });
    }
    ...
    async getNftAddressByIndex(provider: ContractProvider, itemIndex: bigint): Promise<c.Address> {
    ...
    async getNftContent(provider: ContractProvider, itemIndex: bigint, _individualNftContent: string): Promise<CellRef<OffchainMetadataReply>> {
        const r = StackReader.fromGetMethod(1, await provider.get('get_nft_content', [
            { type: 'int', value: itemIndex },
            { type: 'cell', cell: beginCell().storeStringTail(_individualNftContent).endCell() },
        ]));
        return r.readCellRef<OffchainMetadataReply>(OffchainMetadataReply.fromSlice);
    }
    ...
    async getProjectName(provider: ContractProvider): Promise<string> {
        const r = StackReader.fromGetMethod(1, await provider.get('get_project_name', []));
        return r.readSnakeString();
    }

    async getRevealStatus(provider: ContractProvider): Promise<boolean> {
        const r = StackReader.fromGetMethod(1, await provider.get('get_reveal_status', []));
        return r.readBoolean();
    }

    async getBaseUri(provider: ContractProvider): Promise<string> {
